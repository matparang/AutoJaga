...
class BrierScorer:
    ...
    def get_stats(self) -> dict:
        """Get overall Brier scorer statistics for health monitoring."""
        try:
            conn = sqlite3.connect(self.db_path)
            
            # Total predictions
            total = conn.execute(
                "SELECT COUNT(*) FROM brier_outcomes"
            ).fetchone()[0]
            
            # Overall trust score (average across all perspectives)
            cutoff = (
                datetime.now() - timedelta(days=LOOKBACK_DAYS)
            ).isoformat()
            row = conn.execute("""
            ...
            if row and row[0] is not None:
                ...
                brier = (avg_forecast - avg_actual) ** 2
                trust = 1.0 - (brier * 2)
            else:
                trust = 0.5  # neutral if no data
            
            return {
                "total_predictions": total,
                "overall_trust": max(0.0, min(1.0, trust)),
                "recent_brier": brier if row and row[0] else 0.25,
            }
