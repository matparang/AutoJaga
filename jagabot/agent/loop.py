...
class AgentLoop:
    ...
    async def _run_agent_loop(
        ...
        while iteration < max_iterations:
            ...
            _jit = None
            if os.getenv("JAGABOT_JIT_SCHEMAS", "1") == "1":
                try:
                    from jagabot.core.jit_schema import JITSchemaManager
                    _jit = JITSchemaManager(tools_payload)
                    tools_payload = _jit.get_stubs()
                except Exception as _jit_err:
                    logger.debug(f"JIT schema init failed: {_jit_err}")
                    _jit = None
