...
def _make_provider(config):
    ...
    if not p:
        raise RuntimeError(
            ...
            "Set one in ~/.jagabot/config.json under providers section"
        )
...
class SwarmMonitor(Static):
    ...
    WORKERS = 4
    worker_states: reactive[list] = reactive([0.0, 0.0, 0.0, 0.0])
...
class ChatLog(ScrollableContainer):
    ...
    def add_agent(self, text: str) -> None:
        ...
        # Mount response content separately for better formatting
        self.mount(Static(
            text,
            classes="msg-agent-content"
        ))
        self.scroll_end(animate=False)
...
class JagabotTUI(App):
    ...
    async def _init_agent(self) -> None:
        ...
        try:
            ...
            chat = self.query_one("#chat-log", ChatLog)
            chat.add_error(f"Failed to initialize agent: {e}")
