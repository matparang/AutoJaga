# jagabot/agent/outcome_tracker.py
"""
...
Drop into: jagabot/agent/outcome_tracker.py

Wire into loop.py __init__:
    from jagabot.agent.outcome_tracker import OutcomeTracker
    self.outcome_tracker = OutcomeTracker(workspace, tool_registry)

Wire into loop.py _process_message START (session reminder):
    reminder = self.outcome_tracker.get_pending_reminder()
    if reminder:
        # inject reminder into system context or show to user
        logger.info(f"Pending outcomes: {reminder}")

Wire into session_writer.py save():
    # After saving report, extract and log any conclusions
...
class OutcomeTracker:
    ...
    def record_outcome_by_context(
        ...
        # Detect outcome signal
        result = None
        if any(w in msg_lower for w in ["correct", "right", "true", "yes it was"]):
            result = "correct"
        elif any(w in msg_lower for w in ["wrong", "incorrect", "false", "no it wasn't"]):
            result = "wrong"
        elif any(w in msg_lower for w in ["partial", "partly", "somewhat", "mixed"]):
            result = "partial"
