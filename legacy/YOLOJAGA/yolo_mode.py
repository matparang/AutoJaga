...
class YOLORunner:
    ...
    def _execute_step(
        ...
        """
        Execute one autonomous step via agent.

        ─────────────────────────────────────────────────
        REPLACE THIS STUB with real agent call:

        response = await self.agent.process_message(
            prompt,
            yolo_mode=True,          # skip confirmations
            max_tool_calls=10,       # cap per step
            workspace_only=True,     # enforce sandbox
        )

        Parse response for:
        - summary: one-line result
        - findings: count of items found
        - saved_to: file path
        - memory_added: facts added
        - pending_added: conclusions logged
        ─────────────────────────────────────────────────
        """
        import time
        time.sleep(0.8)  # simulate work

        # Stub responses per step type
        stubs = {
            1: {"summary": "5 research angles identified", "findings": 5},
            2: {"summary": "12 sources found and saved",   "findings": 12,
                "saved_to": str(self.workspace / "research_output" /
                               f"step2_{datetime.now().strftime('%H%M%S')}.md")},
            3: {"summary": "8 claims cross-verified",      "findings": 8},
            4: {"summary": "3 key conclusions extracted",  "findings": 3,
                "pending_added": 3},
            5: {"summary": "2 actionable insights found",  "findings": 2},
            6: {"summary": "Memory updated, report saved", "memory_added": 3,
                "saved_to": str(self.workspace / "research_output" /
                               f"yolo_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md")},
        }
        return stubs.get(step_num, {"summary": "Complete", "findings": 0})
