...
from jagabot.channels.formatter import ResponseFormatter
from jagabot.config.schema import TelegramConfig
...
class TelegramChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        """Send a message through Telegram, splitting into multiple parts if needed."""
        if not self._app:
        ...
        # Format into Telegram-safe parts
        formatter = ResponseFormatter()
        parts = formatter.format_for_telegram(msg.content)
        ...
        for i, part in enumerate(parts):
            try:
                ...
                logger.warning(f"HTML parse failed for part {i+1}, falling back to plain text: {e}")
                try:
                    await self._app.bot.send_message(
                        chat_id=chat_id,
                        text=part
                    ...
                    logger.error(f"Error sending Telegram message part {i+1}: {e2}")
            # Small delay between parts for correct ordering
            if i < len(parts) - 1:
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm jagabot.\n\n"
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if media_file and self._app:
            try:
                ...
                media_dir = Path.home() / ".jagabot" / "media"
                ...
                if media_type == "voice" or media_type == "audio":
                    from jagabot.providers.transcription import GroqTranscriptionProvider
