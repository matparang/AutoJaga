...
from jagabot.cli.command_registry import CommandRegistry, CommandContext
...
class EnhancedCLI:
    ...
    def __init__(
        ...
        self.cmd_registry = CommandRegistry(self.workspace)
    ...
    def _process_turn(self) -> None:
        ...
        if user_input.startswith("/"):
            cmd_result = self._handle_registry_command(user_input)
            if cmd_result is None:
                return  # handled locally
            elif isinstance(cmd_result, str):
                # Command returned a response to show
                print_agent_streaming(cmd_result, stream=self.stream)
                return
            # Otherwise continue with agent processing
    ...
    def _handle_registry_command(self, user_input: str):
        """Handle command via registry. Returns response string or None."""
        ...
        if not cmd:
            print_error(f"Unknown command: {user_input}. Type /help for list.")
            return None
        ...
        # Execute command
        try:
            result = cmd.handler(args, ctx) if cmd.handler else f"Command {cmd_name} not yet implemented"
            return result
    ...
    def _call_agent(self, query: str) -> str:
        """
        Call the real AutoJaga AgentLoop.
        """
        import asyncio
        ...
        if self.agent is None:
            try:
                ...
                return "Agent initialization failed. Check config."

        # Hook tool callbacks for live display
        self._captured_tools = []
        
        def on_tool_start(tool_name: str):
            print_tool_start(tool_name)
            self._captured_tools.append(tool_name)

        def on_tool_done(tool_name: str, elapsed: float, status: str = "ok"):
        ...
        try:
            ...
            # Store tools used for ProactiveWrapper
            self._last_tools_used = self._captured_tools
